...
1. What is the DOM?
    ...
    The DOM is what holds all the elements/content and objects that make up web pages that can be manipulated with other languages like JS in order to add more content, style, and functionality to a page.  

2. What is an event?

    An event is something that happens per instructions given in the code for that element as a result of the user doing something on the page or something happening on a page.

3. What is an event listener?

    An event listener is the thing that will make the event happen, once the user triggers it (by clicking, hovering, double-clicking, etc).

4. Why would we convert a NodeList into an Array?

    We would convert a NodeList into an array so we can gain access to manipulate it as we can a regular array. With NodeList, we don’t have many methods we can do on it. We can do .forEach() on a NodeList but not a .reduce() but once converted into an array, we can. 

5. What is a component?

    A component is a chunk of reusable code. It can be used to create multiple elements on a page.
